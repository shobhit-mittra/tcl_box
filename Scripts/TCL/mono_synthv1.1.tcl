...
proc mod_pattern {dirPath pattern searchParam count} {
	...
	# Method to remove repetitions generated in modPattern for bussed elements 
	set single_pattern [lindex [ls_clean [split $modPattern "*"]] 0]

	if {[regexp "${single_pattern}\\*" $modPattern filter]} {
		set modPattern $filter
	}
...
foreach key [dict keys $global_attr] {
	...
	if {$key eq "ck"} {
		...
		while {[info exists ::ck($itemCount,[lindex $ck_ls 0])]} {
			...
			get_attr $ck_ls "ck"  ; # get_attr function output variable syntax : identifier(ck/in/out)_attributeName(as per constraints.csv)_attr
	...
	# Considiering the remaining keys : in and out attributes
		...
		if {$key eq "in"} {
			...
			while {[info exists ::in($itemCount,[lindex $in_ls 0])]} {
				...
				puts "\nWorking on port : $in($itemCount,$in_name) ....."

				puts "\nModified name : $in_name_mod"


				if {$in($itemCount,$in_name) eq $in_name_mod} {
				...
				puts $sdcID "set_input_delay -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -min -rise -source_latency_included $in($itemCount,$in_early_rise_delay_attr) \[get_ports $in_name_mod\]"
				puts $sdcID "set_input_delay -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -min -fall -source_latency_included $in($itemCount,$in_early_fall_delay_attr) \[get_ports $in_name_mod\]"	
				puts $sdcID "set_input_delay -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -max -rise -source_latency_included $in($itemCount,$in_late_rise_delay_attr) \[get_ports $in_name_mod\]"	
				puts $sdcID "set_input_delay -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -max -fall -source_latency_included $in($itemCount,$in_late_fall_delay_attr) \[get_ports $in_name_mod\]"	

				puts $sdcID "set_input_transition -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -min -rise -source_latency_included $in($itemCount,$in_early_rise_slew_attr) \[get_ports $in_name_mod\]"	
				puts $sdcID "set_input_transition -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -min -fall -source_latency_included $in($itemCount,$in_early_fall_slew_attr) \[get_ports $in_name_mod\]"	
				puts $sdcID "set_input_transition -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -max -rise -source_latency_included $in($itemCount,$in_late_rise_slew_attr) \[get_ports $in_name_mod\]"	
				puts $sdcID "set_input_transition -clock \[get_clocks $in($itemCount,$in_clocks_attr)\] -max -fall -source_latency_included $in($itemCount,$in_late_fall_slew_attr) \[get_ports $in_name_mod\]"	
			...
			puts "\nINFO : Done with Input constraints, Proceeding to process Output constraints ....."
		
			set out_ls [dict get $global_attr $key]

			# Processing output constraints information 
			while {[info exists ::out($itemCount,[lindex $out_ls 0])]} {
			
				# Extracting the attribute pointers from in_ls list
				set out_name [lindex $out_ls 0]

				set out_name_mod [mod_pattern $Netlist_Directory $out_name "output" $itemCount]	; # Syntax for mod_pattern command : mod_pattern <directory_path_to_search> <pattern_to_search> <search_word_associated_in_verilog> <item_count>
				puts "\nWorking on port : $out($itemCount,$out_name) ....."

				puts "\nModified name : $out_name_mod"


				if {$out($itemCount,$out_name) eq $out_name_mod} {
					puts "\nINFO : output port : $out($itemCount,$out_name) is not a bus"
			
				} else {
					puts "\nINFO : output port : $out($itemCount,$out_name) is a bus"
				}
				...
				# Output constraints : 
				# 	set_output_delay -clock [get_clocks <clock_name>] -min/-max -rise/-fall -source_latency_included <value> [get_ports <output_port>] 
				# 	set_load <load_value> [get_ports <output_port>]		

			
				puts $sdcID "set_output_delay -clock \[get_clocks $out($itemCount,$out_clocks_attr)\] -min -rise -source_latency_included $out($itemCount,$out_early_rise_delay_attr) \[get_ports $out_name_mod\]"
				puts $sdcID "set_output_delay -clock \[get_clocks $out($itemCount,$out_clocks_attr)\] -min -fall -source_latency_included $out($itemCount,$out_early_fall_delay_attr) \[get_ports $out_name_mod\]"	
				puts $sdcID "set_output_delay -clock \[get_clocks $out($itemCount,$out_clocks_attr)\] -max -rise -source_latency_included $out($itemCount,$out_late_rise_delay_attr) \[get_ports $out_name_mod\]"	
				puts $sdcID "set_output_delay -clock \[get_clocks $out($itemCount,$out_clocks_attr)\] -max -fall -source_latency_included $out($itemCount,$out_late_fall_delay_attr) \[get_ports $out_name_mod\]"	

				puts $sdcID "set_load $out($itemCount,$out_load_attr) \[get_ports $out_name_mod\]"	
...
puts "\nSUCCESS : Done with Output constraints. All constranits successfully converted into SDC. Please check the path : ${Netlist_Directory} to find the generated SDC file - 'constraints.sdc'"
